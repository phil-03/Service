...
void ServiceFailure() {
   ...
   //Maybe query curent failure config options before hand?
   LPSERVICE_FAILURE_ACTIONS lpfa = NULL;
   ...
   {
      if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
         cbBufSize = dwBytesNeeded; //Specify the amount of bytes needed to retrieve config info
         lpfa = (LPSERVICE_FAILURE_ACTIONS)LocalAlloc(LMEM_FIXED, cbBufSize);
      }
      else {
         cout << "Error No = " << GetLastError() << endl;
         CloseServiceHandle(schService);
   ...
   CloseServiceHandle(schService);//will reopen for failure config

   SERVICE_FAILURE_ACTIONS sfa; //Service controller for failure actions
   LPWSTR rebootMsg = lpfa->lpRebootMsg;
   LPWSTR lpcmd = lpfa->lpCommand;
   DWORD resetPeriod = lpfa->dwResetPeriod;

   DWORD ac = lpfa->cActions;
   sfa.lpsaActions = lpfa->lpsaActions;
   ...
   if (lstrcmpiA(option, "rebootMsg") == 0) {
      ...
      sfa.lpRebootMsg = lpcmd;

   }
   else  if (lstrcmpiA(option, "resetPeriod") == 0) {

      //Preset to 30
      sfa.dwResetPeriod = 30; //Need to convert to user input ***
      ...
      SC_ACTION_TYPE type[5];
      SC_ACTION fActions[10];
      //Need to still specify delay time
      ...
      if (lstrcmpiA(option3, "reboot") == 0) {
         ...
         type[2] = SC_ACTION_REBOOT; // control variable
         fActions[2].Type = type[2];
         ac = 2;
         sfa.lpsaActions = fActions; //Set value for action
      }
      else if (lstrcmpiA(option3, "restart") == 0) {

         type[1] = SC_ACTION_RESTART; // control variable
         fActions[1].Type = type[1];
         ac = 1;
         sfa.lpsaActions = fActions; //Set value for action
      }
      else if (lstrcmpiA(option3, "command") == 0) {

         type[3] = SC_ACTION_RUN_COMMAND; // control variable
         fActions[3].Type = type[3];
         ac = 3;
         sfa.lpsaActions = fActions; //Set value for action
      }
      else {

         cout << "Sorry that option isn't available..." << endl;

      }

      if (lstrcmpiA(option3, "none") == 0) {

         type[0] = SC_ACTION_NONE;
         fActions[0].Type = type[0];
         ac = 0;
         sfa.lpsaActions = fActions; //Set value for action
      }
      ...
      sfa.cActions = ac;//#of actions


      //Open Service Once more
      schService = OpenService(
         schOpenSCManager,
         input,
